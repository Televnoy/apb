// Используем деструктуризацию из глобального объекта React, так как мы работаем без сборщика
const { useState, useEffect, useRef } = React;

// Маппинг иконок из глобального объекта Lucide
const { 
...
const App = () => {
  ...
  // Сообщаем index.html, что приложение готово к показу
  useEffect(() => {
  ...
  const getAvailableScores = () => Array.from({ length: 30 }, (_, i) => 30 - i);
  ...
  return (
    <div className="min-h-screen bg-[#F9F9F9] text-[#2C2C2C] pb-48 relative font-sans">
      {toast && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[100] bg-red-600 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3">
          <AlertCircle size={18} />
      ...
      <main className="max-w-md mx-auto px-6 pt-8 space-y-10">
        ...
        {participants.map((participant, index) => {
          ...
          return (
            <div key={participant.id} className={`group relative ${isCurrentSubmitted ? 'opacity-75' : ''}`}>
              <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-medium text-[#BBB] font-mono uppercase">Участник #{String(index + 1).padStart(2, '0')}</span>
              ...
              <div className="bg-white rounded-2xl p-6 border border-[#EAEAEA] shadow-[0_4px_20px_rgba(0,0,0,0.02)] space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <div className="relative">
                    ...
                    <button disabled={isCurrentSubmitted} onClick={() => setOpenSelect({ id: participant.id, type: isNumOpen ? null : 'number' })} className="w-full flex items-center justify-between border-b border-[#EEE] pb-2 text-[18px] font-mono font-light outline-none">
                      {participant.number} {!isCurrentSubmitted && <ChevronDown size={14} className="text-[#DDD]" />}
                    </button>
                    {isNumOpen && (
                      <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-[#EEE] z-[80] h-48 overflow-y-auto shadow-xl rounded-lg">
                        {participantNumbers.map(n => {
                          const isOcc = allOccupiedNumbers.includes(n) && n !== participant.number && n !== '00';
                          return <div key={n} onClick={() => !isOcc && updateParticipant(participant.id, 'number', n)} className={`py-2.5 px-4 font-medium border-b border-[#F5F5F5] text-[14px] font-mono ${isOcc ? 'text-red-300 bg-gray-50' : 'hover:bg-[#F9F9F9]'}`}>{n} {isOcc && '✖'}</div>
                        })}
                      </div>
                    )}
                  </div>
                  <div className="relative">
                    ...
                    <button disabled={isCurrentSubmitted} onClick={() => setOpenSelect({ id: participant.id, type: isScoreOpen ? null : 'score' })} className={`w-full flex items-center justify-between border-b pb-2 text-[18px] font-mono font-light outline-none ${participant.score ? 'border-black text-black' : 'border-[#EEE] text-[#CCC]'}`}>
                    ...
                    {isScoreOpen && (
                      <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-[#EEE] z-[80] h-48 overflow-y-auto shadow-xl rounded-lg">
                        ...
                        {availableScores.map(s => {
                          ...
                          return <div key={s} onClick={() => !isOcc && updateParticipant(participant.id, 'score', s)} className={`py-2.5 px-4 font-medium border-b border-[#F5F5F5] text-[14px] font-mono ${isOcc ? 'text-red-300 bg-gray-50' : 'hover:bg-[#F9F9F9]'}`}>{s} {isOcc && '✖'}</div>
                        })}
                      </div>
                    )}
                  </div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-[#EEE] px-6 py-4 pb-12 z-40">
        <div className="max-w-md mx-auto">
          <div className="flex gap-2 mb-4 overflow-x-auto py-2 px-1">
            {availableScores.map(s => {
              const used = occupiedScores.includes(s);
              return <div key={s} className={`flex-shrink-0 w-6 h-6 flex items-center justify-center text-[8px] rounded-full border transition-all duration-300 ${used ? 'bg-black border-black text-white scale-110' : 'bg-transparent border-[#EEE] text-[#CCC]'}`}><span className="font-mono">{s}</span></div>
            })}
          </div>
...
// Инициализация React приложения в DOM
